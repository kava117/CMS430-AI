...
class State(object):
    ...
    def __lt__(self, other):
        """Tiebreaker for PriorityQueue when two states have equal priority."""
        return self.total_moves < other.total_moves

    def get_neighbors(self):
        """Generate successor states for up/down/left/right moves."""
        neighbors = []
...
def main():
    ...
    while not queue.empty():
        ...
        if state.position == goal_position:
            print('Found a path with %d moves.' % state.total_moves)
        ...
        for neighbor in state.get_neighbors():
            if neighbor.position not in visited:
                priority = neighbor.total_moves + neighbor.manhattan_distance()
